...
export const jwtConfig = {
  secret:
    ...
    "your_jwt_secret_key_change_in_production_min_32_chars",
...
export const refreshConfig = {
  // Route simulation update interval (driver movement on map)
  routeSimulationInterval: parseInt(
    process.env.ROUTE_SIMULATION_INTERVAL_MS || "3000",
    10
  ),

  // Dashboard summary refresh interval
  dashboardSummaryInterval: parseInt(
    process.env.DASHBOARD_SUMMARY_INTERVAL_MS || "3000",
    10
  ),

  // Driver location share interval (how often driver sends location)
  driverLocationShareInterval: parseInt(
    process.env.DRIVER_LOCATION_SHARE_INTERVAL_MS || "3000",
    10
  ),

  // Route refresh delay after status change (frontend)
  routeRefreshDelay: parseInt(process.env.ROUTE_REFRESH_DELAY_MS || "1000", 10),

  // Location update throttle delay (frontend)
  locationUpdateThrottle: parseInt(
    process.env.LOCATION_UPDATE_THROTTLE_MS || "5000",
    10
  ),

  // Dashboard summary refresh throttle (frontend)
  dashboardSummaryThrottle: parseInt(
    process.env.DASHBOARD_SUMMARY_THROTTLE_MS || "2000",
    10
  ),
};
