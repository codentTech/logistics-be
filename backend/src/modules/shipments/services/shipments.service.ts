...
export class ShipmentService {
  ...
  async assignDriver(
    ...
    try {
      ...
      ShipmentStateMachine.validateTransition(
        shipment.status,
      ...
      shipment.pendingApproval = true; // Set pending approval flag
      shipment.approvedAt = null; // Reset approval timestamp
      // Clear cancelledAt if reassigning from cancelled status
      if (
        previousStatus === ShipmentStatus.CANCEL_BY_DRIVER ||
      ...
      // Get driver's user ID to send notification
      const driverWithUser = await driverRepository.findOne({
        where: { id: assignDto.driverId, tenantId },
        relations: ["user"],
      });
      ...
      if (!shipmentWithRelations) {
        throw new AppError(
          ErrorCode.SHIPMENT_NOT_FOUND,
          "Shipment not found after update",
          404
        );
      }
  ...
  async updateStatus(
    ...
    try {
      ...
      if (
        updateDto.status === ShipmentStatus.DELIVERED &&
  ...
  async getAllShipments(
    ...
    if (userRole === UserRole.DRIVER && driverId) {
      shipments = await this.shipmentRepository.findByDriver(
        tenantId,
        driverId
      );
    } else {
  ...
  async getShipmentById(
    ...
    // Exception: Allow access if shipment status is CREATED and driverId is null
    // (driver may have just rejected/cancelled, and they should see the result)
    if (userRole === UserRole.DRIVER && driverId) {
      // Allow access if shipment has no driver assigned (CREATED status after rejection/cancellation)
      // OR if the driver is assigned to this shipment
      if (shipment.driverId !== null && shipment.driverId !== driverId) {
        throw new AppError(
          ErrorCode.UNAUTHORIZED,
  ...
  async cancelByCustomer(
    ...
    try {
      ...
      shipment.assignedAt = null; // Clear assignment timestamp
  ...
  /**
   * Driver approves shipment assignment
   */
  async approveAssignment(
    ...
    try {
      ...
      if (shipment.driverId !== driverId) {
        throw new AppError(
          ErrorCode.UNAUTHORIZED,
          "You can only approve shipments assigned to you",
          403
        );
      }

      // Verify shipment is in ASSIGNED status with pending approval
      if (
        shipment.status !== ShipmentStatus.ASSIGNED ||
        ...
        throw new AppError(
          ErrorCode.INVALID_SHIPMENT_STATE,
          "Shipment is not pending approval",
          400
        );
      }
      ...
      shipment.pendingApproval = false;
      ...
      const history = await this.shipmentRepository.addStatusHistory({
        ...
        changedBy: userId,
        changedAt: new Date(),
        metadata: { approvedBy: "driver", driverId },
      });
      await queryRunner.manager.save(history);
  ...
  /**
   * Driver rejects shipment assignment (reverts to CREATED)
   */
  async rejectAssignment(
    ...
    try {
      ...
      if (shipment.driverId !== driverId) {
        throw new AppError(
          ErrorCode.UNAUTHORIZED,
          "You can only reject shipments assigned to you",
          403
        );
      }
      ...
      // Validate state transition (ASSIGNED → CREATED)
      ShipmentStateMachine.validateTransition(
        shipment.status,
      ...
      // Use update to explicitly set null values in database
      await queryRunner.manager.update(
        "Shipment",
        { id: shipment.id, tenantId },
        {
          status: ShipmentStatus.CREATED,
      ...
      // Reload the shipment to get the updated entity from database
      const reloadedShipment = await this.shipmentRepository.findById(
        shipment.id,
        tenantId
      );
      ...
      // Explicitly ensure driverId and driver are null (in case relation was loaded)
      reloadedShipment.driverId = null;
      reloadedShipment.assignedAt = null;
      reloadedShipment.driver = null;

      const updatedShipment = reloadedShipment;
      const history = await this.shipmentRepository.addStatusHistory({
        shipmentId: updatedShipment.id,
        status: ShipmentStatus.CREATED,
        changedBy: userId,
        changedAt: new Date(),
        metadata: { rejectedBy: "driver", previousDriverId: driverId },
      });
      await queryRunner.manager.save(history);
  ...
  private getEventTypeForStatus(status: ShipmentStatus): ShipmentEventType {
    const statusToEvent: Record<ShipmentStatus, ShipmentEventType> = {
      ...
      [ShipmentStatus.APPROVED]: ShipmentEventType.STATUS_CHANGED,
